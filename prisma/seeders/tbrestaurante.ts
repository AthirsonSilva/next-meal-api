...
export const tbTipoRestauranteSeed = async () => {
	for (let i = 0; i < 10; i++) {
...
const tbRestauranteSeed = async () => {
	...
	for (let i = 0; i < 10; i++) {
		await prisma.tbrestaurante.create({
			data: {
				...
				idTipoRestaurante: parseInt(faker.random.numeric(1)),
				tbmesa: {
					create: {
